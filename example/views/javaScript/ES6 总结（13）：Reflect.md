---
title: ES6 总结（13）：Reflect
date: 2021-08-25 11:11:11
tags:
  - JavaScript
  - ES6+
categories:
...
（4）`Reflect` 对象的方法与 `Proxy` 对象的方法一一对应，只要是 `lect` 对象上找到对应的方法。这就让 `Proxy` 对象可以方便地调用对应的 `Reflect` 方法，完成默认行为，作为修改行为的基础。也就是说，不管 `Proxy` 怎么修改默认行为，你总可以在 `Reflect` 上获取默认行为。
